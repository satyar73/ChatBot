 #!/bin/sh
  ...
  CONTAINER_BACKEND="backend:8005"
  HOST_BACKEND="host.docker.internal:8005"
  ...
  if check_backend $CONTAINER_BACKEND; then
    echo "Using container backend at $CONTAINER_BACKEND"
    BACKEND_URL=$CONTAINER_BACKEND
  else
    echo "Container backend not available, trying host backend at $HOST_BACKEND"
    if check_backend $HOST_BACKEND; then
      echo "Using host backend at $HOST_BACKEND"
      BACKEND_URL=$HOST_BACKEND
    else
      echo "WARNING: Neither backend is available! Will try to use host backend but it may not work."
      BACKEND_URL=$HOST_BACKEND
    fi
  fi
